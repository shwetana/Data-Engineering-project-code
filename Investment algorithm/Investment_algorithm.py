...
############### List of 50 NASDAQ companies ###############
#list_cmp = ["AAPL", "ADBE", "ADI", "ADP", "ADSK", "ALGN", "ALXN", "AMAT", "AMD", "AMGN", "AMZN", "ASML", "ATVI", "AVGO", "BIIB", "BMRN", "CDNS", "CERN", "CHKP", "CMCSA", "COST", "CSCO", "CSX", "CTAS", "CTSH", "CTXS", "EBAY", "EXPE", "GOOG", "GOOGL", "IDXX", "ILMN", "INCY", "INTC", "INTU", "ISRG", "JD", "LBTYA", "LBTYK", "LRCX", "MAR", "MCHP", "MELI", "MSFT", "MXIM"]
#list_cmp = ["AAPL", "ADBE", "ADI", "ADP", "ADSK", "ALGN", "ALXN", "AMAT", "AMD", "AMGN", "AMZN", "ASML", "ATVI", "AVGO", "BIIB", "BMRN", "CDNS", "CERN", "CHKP", "CMCSA", "COST", "CSCO", "CSX", "CTAS", "CTSH", "CTXS", "EBAY", "EXPE", "FAST", "FB", "FISV", "FOX", "GILD", "GOOG", "GOOGL", "IDXX", "ILMN", "INCY", "INTC", "INTU", "ISRG", "JD", "LBTYA", "LBTYK", "LRCX", "MAR", "MCHP", "MELI", "MSFT", "MXIM"]
...
############### File path ################
#path = 'F:\\SRH Academics\\Data Engineering\\Project\\Data\\6-7-20'

############ To extract data from API #############
# API_KEY ="T51VUT68CNVQY7XA"
# for cmp in list_cmp:
#     response = requests.get('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=' + cmp + '&apikey=' + API_KEY)
#     result = response.json()
#     if(response.status_code == 200):
#         with open(f"{path}\\{cmp}.json", "w") as cmp:
#             json.dump(result, cmp)
#             time.sleep(30)
#     else:
#         print("Could not fetch data from API. Error occured: {cmp.status_code}")
...
#     insert_many_records("stockmarkets","stocks",df)
...
def naming(company_name, collection_name):
    ...
    # col = db["StocksDaily"]
    # data = col.find({ "Symbol" : company_name})  
    # df = pd.DataFrame(list(data))
    # df = df[-300:]
    # df.reset_index(drop = True, inplace = True)
    #data={}
    # with open(f'{path}\\{company_name}.json') as jsonfile:
    #     data = json.load(jsonfile)
    # metadata1 = data['Time Series (Daily)']
    ...
    #df.reset_index(drop = True, inplace = True)
    # df = df.rename(columns={"_id":"id", "Date":"date", "Open":"open", "High":"high", "Low":"low", "Close":"close", "Volume":"volume", "Symbol":"symbol"})
    ...
    list_metadata_dates = []
    list_company_names =[]
    cursor = col.find({"Meta Data" : {"2. Symbol" : company_name}})
    ...
    for eachValue in cursor:
        ...
        list_company_names.append(metadata2)
        print(metadata1)
        df = pd.DataFrame.from_dict(metadata1, orient='index')
        df['date'] = df.index.values
        list_names = []
...
print(od["MXIM"])
#print(od[list_cmp[0]])
#print(od["FOX"].tail(30))
#print(od["MSFT"][-100:])
#print(od["MSFT"][od["MSFT"]['date' == ['2020-06-03', '2020-06-10', '2020-06-17', '2020-06-24', '2020-07-01']]])
...
thisweek = '08072020'
lastweek = '01072020'
# thisweek_cal = datetime.datetime.strptime(thisweek, "%d%m%Y")
# DD = datetime.timedelta(days=7)
# earlier = thisweek_cal - DD
# lastweek = earlier.strftime("%d%m%Y")
#print(lastweek)
...
def buy_stocks():
    ...
    for i in range(top):
        stock_count = 0
        for stsz in range(df_eligible['StockSize'][i]): #0 to 127
            amount = df_eligible['CurrentPrice'][i] #22
            if amount <= balance: #5000 > 22, 4978 > 22, ...
                balance -= amount          # bal = 5000 - 22 = 4978, 4978 - 22 = 4956, ....  
                stock_count += 1
...
def linearregression(company_name):
    ...
    ######################## Plotting ###########################
    plt.scatter(x, log_y, s = 5, marker = ".", label = 'original data')
    plt.xticks(rotation=90)
    plt.plot(x, y_fit, c = 'g', label = 'fitted line')
    plt.xlabel('Number of days')
    plt.ylabel('Closing prices')
    plt.legend()
    plt.show()

#linearregression("AMZN")